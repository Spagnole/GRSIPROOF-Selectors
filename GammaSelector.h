...
#ifndef GammaSelector_h
#define GammaSelector_h
...
class GammaSelector : public TGRSISelector { //Must be same name as .C and .h
 ...
 GammaSelector(TTree * /*tree*/ =0) : TGRSISelector(), fGrif(NULL), fBgo(NULL), fZds(NULL), fScep(NULL), fPaces(NULL), fLaBr(NULL), fLaBrBgo(NULL), fTAC(NULL), fDesc(NULL) {
      SetOutputPrefix("S1916Griffin_"); //Changes prefix of output file
   }
	//These functions are expected to exist
   virtual ~GammaSelector() { }
   virtual Int_t   Version() const { return 2; }
   void CreateHistograms();
   void FillHistograms();
   void InitializeBranches(TTree *tree);

   ClassDef(GammaSelector,2); //Makes ROOT happier
};

#endif

#ifdef GammaSelector_cxx
void GammaSelector::InitializeBranches(TTree* tree)
{
   if(!tree) return;
