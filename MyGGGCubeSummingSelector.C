#define MyGGGCubeSummingSelector_cxx
// The class definition in MyGGGCubeSummingSelector.h has been generated automatically
#include "MyGGGCubeSummingSelector.h"

int kValue_grif = 379; // default k-value for not piled-up hits

void MyGGGCubeSummingSelector::CreateHistograms() {
	// define Trees
...
//opposite detector check for summing correction matrices
bool IsOppositeCrystal(TGriffinHit* Hit1, TGriffinHit* Hit2)
{
   TVector3 Vec1 = TGriffin::GetPosition(Hit1->GetDetector(), Hit1->GetCrystal(), 145.);
   TVector3 Vec2 = TGriffin::GetPosition(Hit2->GetDetector(), Hit2->GetCrystal(), 145.);
   return TMath::RadToDeg()*(Vec1.Angle(Vec2)) > 179.5;
} 

void MyGGGCubeSummingSelector::FillHistograms() {
	...
	if( fGrif->GetSuppressedMultiplicity(fGriffinBgo) > 2 && fGrif->GetSuppressedMultiplicity(fGriffinBgo) < 8){	
		for(auto g1 = 0; g1 < fGrif->GetSuppressedMultiplicity(fGriffinBgo); ++g1) {
			...
			for( auto g2 = 0; g2 < fGrif->GetSuppressedMultiplicity(fGriffinBgo); ++g2){
				if( g1 == g2 ) continue;
				auto grif2 = fGrif->GetSuppressedHit(g2);
				if(grif2->GetKValue() != kValue_grif) continue;
				gE[1] = grif2->GetEnergy();
				//for( auto g3 = 0; g3 < fGrif->GetSuppressedMultiplicity(fGriffinBgo); ++g3){
				for( auto g3 = g2+1; g3 < fGrif->GetSuppressedMultiplicity(fGriffinBgo); ++g3){
					...
					if( !IsOppositeCrystal(grif2,grif3) ) continue;
					...
					*/
					...
					if( gE[0] > 0. && gE[1] > 0. && gE[2] > 0.){
						...
						//reverse order of tree branches
						gE[2] = grif2->GetEnergy();
						gE[1] = grif3->GetEnergy();
						//filling tree with reversed gamma2-gamma3 energies for symmetry
						fTree.at("ggg_tree")->Fill();
					}
				}
